...
Regression target (April 2026): Kimi Coding Plan's ``api.kimicode.com/coding``
...
@pytest.mark.parametrize("url,expected,label", [
    ...
    ("https://api.kimicode.com/coding/v1", True, "Kimi Code CLI /v1"),
    ("https://api.kimicode.com/coding", True, "Kimi Code CLI no /v1"),
...
def test_maybe_wrap_anthropic_rewraps_kimi_coding_url():
    """Plain OpenAI client pointed at api.kimicode.com/coding gets rewrapped."""
    ...
    with patch(
        ...
        result = _maybe_wrap_anthropic(
            ...
            "https://api.kimicode.com/coding", api_mode=None,
        )
...
def test_resolve_provider_client_kimi_coding_wraps_anthropic(monkeypatch, tmp_path):
    ...
    assert isinstance(client, AnthropicAuxiliaryClient), (
        "Kimi Coding Plan endpoint (api.kimicode.com/coding) speaks Anthropic "
        "Messages — aux client MUST be AnthropicAuxiliaryClient, got "
        f"{type(client).__name__}"
    )
    assert "kimicode.com/coding" in str(client.base_url)
